...
データベースは MariaDB（Prisma の `mysql` provider）を使用します。文字列は `utf8mb4` で保存されます。金額は整数 JPY、郵便番号と電話番号は記号なしで保存します。位置情報は MVP では Decimal 緯度経度です。大規模化時は MariaDB の spatial column と spatial index を migration で追加します。
